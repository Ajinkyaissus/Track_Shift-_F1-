...
Fast in-memory serving layer with zero disk I/O on request paths.
"""
...
class TyreDebtService:
    ...
    async def get_ledger(self, stint_id: str) -> Dict[str, Any]:
        ledgers = self.app_data.get("ledger", {})
        if stint_id not in ledgers:
    ...
    async def get_attribution(self, stint_id: str) -> Dict[str, Any]:
        feature_means = self.app_data.get("stint_feature_means", {})
        if stint_id not in feature_means:
        ...
        async def _compute():
            means = feature_means[stint_id]
            track_id = self.app_data.get("stint_track_map", {}).get(stint_id)
            ...
            coefs = self.app_data.get("coefficients", {})
            for feature, avg_val in means.items():
                if math.isnan(avg_val):
                    avg_val = 0.0
                    
                coef = coefs.get((stage3_version, feature, track_id))
                if coef is None:
                    coef = coefs.get((stage3_version, feature, None))
                if coef is None:
            ...
            deg_per_lap = self.app_data.get("stint_avg_loss_per_lap", {}).get(stint_id, 0.1)
