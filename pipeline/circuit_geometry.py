...
directly from FastF1 sessions (CircuitInfo & reference telemetry traces) for the 13 target circuits.
"""
import os
import sys
import sqlite3
...
# 13 Target Circuits Specification
CIRCUITS_SPEC = [
    {
        "circuit_id": "bahrain",
...
def extract_and_store_circuit_geometry():
    print("=================================================================")
    print("  EXTRACTING REAL F1 CIRCUIT GEOMETRY (13 TARGET CIRCUITS)       ")
    print("=================================================================")
    init_db()
    ...
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM circuit_corners") # Refresh
    ...
    for spec in CIRCUITS_SPEC:
        ...
        try:
            ...
            print(f"  [Error] Failed to extract geometry for {cid}: {e}")
            cursor.execute("""
                INSERT OR REPLACE INTO tracks (track_id, name, country, country_code, location, rotation, map_available, telemetry_available)
                VALUES (?, ?, ?, ?, ?, 0, 0, 0)
    ...
    if all_trace_points:
        ...
        print(f"\n[OK] Successfully saved {len(full_geom_df)} total geometry points to {GEOMETRY_PARQUET}")
    else:
